...
class AtpMatchSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        #home
            ...
            home = self.get_teamname_from_str(str)
        #away
            str = match_item.xpath('.//div[@class="score_away_txt score_cell wrap"]/span/text()').extract_first().strip()
            away = self.get_teamname_from_str(str)
        ...
        #round scores    
            ...
            home_r1 = self.get_score_from_str(home_r1_t)
            home_r2 = self.get_score_from_str(home_r2_t)
            home_r3 = self.get_score_from_str(home_r3_t)
            home_r4 = self.get_score_from_str(home_r4_t)
            home_r5 = self.get_score_from_str(home_r5_t)

            away_r1 = self.get_score_from_str(away_r1_t)
            away_r2 = self.get_score_from_str(away_r2_t)
            away_r3 = self.get_score_from_str(away_r3_t)
            away_r4 = self.get_score_from_str(away_r4_t)
            away_r5 = self.get_score_from_str(away_r5_t)
        #total games    
            totalgames = 0
    ...
    def get_score_from_str(self, str):
        val = -1
        if str != '-':
            val = int(str, 10)
        return val
